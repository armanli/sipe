const materials = [
  ...
  },
  {
    ...
    aiUses: ['Planejamento Bimestral', 'Avaliações Práticas', 'Sugestões Criativas'],
    aiSuggestions: [
      {
        icon: 'ti-camera',
  ...
  },
  {
    ...
    aiUses: ['Geração de Avaliações', 'Formatação Automática', 'Alinhamento Curricular'],
    aiSuggestions: [
      {
        icon: 'ti-sparkles',
...
document.addEventListener('DOMContentLoaded', () => {
  ['kpi1', 'kpi2', 'kpi3', 'kpi4'].forEach((id, i) => {
    setTimeout(() => document.getElementById(id).classList.add('visible'), 100 + i * 100);
  });
  document.querySelectorAll('.upload-item.uploaded .upload-progress-fill').forEach((el) => {
...
function openPanel(id, autoSuggest) {
  ...
  selectedId = id;
  document.querySelectorAll('.materials-card').forEach((c) => c.classList.remove('selected'));
...
function simulateUpload(item) {
  if (item.classList.contains('uploaded')) {
    showToast('Material já indexado e disponível na IA.', 'success', 'ti-circle-check');
    return;
  }
  item.classList.add('uploaded');
  item.querySelector('i').className = 'ti ti-cloud-check';
  item.querySelector('.upload-item-name').textContent =
    item.querySelector('.upload-item-name').textContent;
  ...
  setTimeout(() => {
    bar.style.width = '100%';
    item.querySelector('.upload-item-desc').textContent = 'Indexado com sucesso';
    item.querySelector('i').className = 'ti ti-circle-check';
    showToast('Material indexado pela IA com sucesso!', 'success', 'ti-database-import');
  }, 1500);
}
